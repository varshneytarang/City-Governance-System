"""
PHASE 6.5: Coordination Checkpoint Node (PROACTIVE) - Fire Agent
...
def coordination_checkpoint_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        resources_needed = []
        incident_type = input_event.get("incident_type", "")
        
        if "fire" in incident_type.lower() or "hazmat" in incident_type.lower():
            resources_needed.append(f"fire_crews_{location}")
            resources_needed.append(f"fire_equipment_{location}")
        if "rescue" in incident_type.lower():
            resources_needed.append(f"rescue_team_{location}")
        resources_needed.append(f"emergency_access_{location}")
