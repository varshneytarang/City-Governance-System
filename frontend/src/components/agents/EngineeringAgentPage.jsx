...
import { 
  ...
  Workflow, PlayCircle, FileText, Shield
} from 'lucide-react'
...
const EngineeringAgentPage = () => {
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        const response = await fetch(`${API_BASE}/api/task-orchestration/recent-activities/engineering?limit=10`)
        if (response.ok) {
          const data = await response.json()
                ...
                <div className="bg-gradient-to-r from-blue-500 to-cyan-500 p-6 text-white">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-blue-100 text-sm">Completed workflows</p>
                    </div>
                    ...
                    recentActivities.slice(0, 6).map((activity, index) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="backdrop-blur-sm bg-white/80 border border-cyan-200 rounded-2xl p-4 hover:bg-white hover:shadow-md transition-all cursor-pointer"
                        >
