...
def confidence_estimator_node(state: DepartmentState) -> DepartmentState:
    ...
    try:
        ...
        if llm_client and settings.USE_LLM_FOR_CONFIDENCE:
...
def _estimate_with_llm(client, feasible: bool, policy_ok: bool, risk_level: str, 
    ...
    try:
        prompt = f"""Assess confidence in this Sanitation Department decision.
...
Consider:
1. Feasibility and policy compliance (higher = more confident)
2. Risk level (higher risk = lower confidence)
3. Number of retry attempts (more attempts = lower confidence)
4. Data completeness and quality
5. Public health and safety implications"""

        response = client.chat.completions.create(
            model=settings.LLM_MODEL,
            messages=[
                {"role": "system", "content": "You are a Sanitation Department confidence assessment AI. Provide realistic confidence scores for operational decisions. Always return valid JSON."},
