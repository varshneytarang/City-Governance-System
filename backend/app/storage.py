...
def log_decision(agent_id: str, request: Dict[str, Any], response: Dict[str, Any], job_id: Optional[str] = None) -> str:
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO agent_decisions (id, agent_id, request_json, response_json, confidence, created_at, job_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
        (
            decision_id,
            agent_id,
            json.dumps(request),
...
def list_decisions(limit: int = 50) -> List[Dict[str, Any]]:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("SELECT id, agent_id, request_json, response_json, confidence, created_at, job_id FROM agent_decisions ORDER BY created_at DESC LIMIT ?", (limit,))
    rows = cur.fetchall()
    conn.close()
...
def get_decision_record(decision_id: str) -> Optional[Dict[str, Any]]:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("SELECT id, agent_id, request_json, response_json, confidence, created_at, job_id FROM agent_decisions WHERE id = ?", (decision_id,))
    r = cur.fetchone()
    conn.close()
    if not r:
        return None
