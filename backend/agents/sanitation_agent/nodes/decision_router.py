...
def _route_with_llm(client, feasible: bool, policy_ok: bool, confidence: float,
    ...
    try:
        prompt = f"""Make a routing decision for this Sanitation Department operational request.
...
SAFETY CONCERNS: {state.get('safety_concerns', [])}
...
Decision criteria:
1. RECOMMEND if: feasible, policy compliant, confidence >= 0.70, risk is low/medium
2. ESCALATE if: not feasible, policy violated, confidence < 0.70, risk is high/critical, public health threat

Consider public health and safety as top priority."""

        response = client.chat.completions.create(
            model=settings.LLM_MODEL,
            messages=[
                {"role": "system", "content": "You are a Sanitation Department routing AI. Make intelligent decisions about whether to proceed with recommendations or escalate to humans. Always return valid JSON."},
        ...
        should_escalate = result.get("should_escalate", True)
