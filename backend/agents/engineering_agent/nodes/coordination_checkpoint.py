"""
PHASE 6.5: Coordination Checkpoint Node (PROACTIVE) - Engineering Agent
...
def coordination_checkpoint_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        resources_needed = []
        project_type = input_event.get("project_type", "")
        
        if "contractor" in plan or project_type:
            resources_needed.append(f"contractors_{location}")
        if "equipment" in plan:
            resources_needed.append(f"equipment_{location}")
        if estimated_cost > 0:
