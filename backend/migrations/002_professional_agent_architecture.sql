/* =========================================================
   PROFESSIONAL AGENT ARCHITECTURE MIGRATION
   Agent Decision Audit Trail + Supporting Tables
   Database: city_mas
   Run after: 001_water_fire_agents.sql
   ========================================================= */
...
CREATE INDEX IF NOT EXISTS idx_agent_decisions_request_type ON agent_decisions(request_type);
...
CREATE INDEX IF NOT EXISTS idx_agent_decisions_feasible ON agent_decisions(feasible);
...
CREATE INDEX IF NOT EXISTS idx_budget_department ON department_budgets(department);
CREATE INDEX IF NOT EXISTS idx_budget_period ON department_budgets(year, month);
CREATE INDEX IF NOT EXISTS idx_budget_status ON department_budgets(status);


/* ---------- PROJECT TRACKING (FOR BUDGET QUERIES) ---------- */
...
/* ---------- WORK SCHEDULES (FOR CONSTRAINT CHECKING) ---------- */
...
CREATE INDEX IF NOT EXISTS idx_schedules_department ON work_schedules(department);
CREATE INDEX IF NOT EXISTS idx_schedules_date ON work_schedules(scheduled_date);
CREATE INDEX IF NOT EXISTS idx_schedules_location ON work_schedules(location);
CREATE INDEX IF NOT EXISTS idx_schedules_status ON work_schedules(status);
CREATE INDEX IF NOT EXISTS idx_schedules_priority ON work_schedules(priority);
...
/* ---------- PIPELINES TABLE (ENHANCED FOR WATER AGENT) ---------- */
...
CREATE INDEX IF NOT EXISTS idx_pipelines_location ON pipelines(location);
CREATE INDEX IF NOT EXISTS idx_pipelines_zone ON pipelines(zone);
CREATE INDEX IF NOT EXISTS idx_pipelines_status ON pipelines(operational_status);
CREATE INDEX IF NOT EXISTS idx_pipelines_pressure ON pipelines(pressure_psi);


/* ---------- RESERVOIRS TABLE (FOR BACKUP CHECKS) ---------- */
...
CREATE INDEX IF NOT EXISTS idx_reservoirs_location ON reservoirs(location);
CREATE INDEX IF NOT EXISTS idx_reservoirs_status ON reservoirs(operational_status);


/* ---------- INCIDENTS TABLE (FOR SAFETY CHECKS) ---------- */
...
CREATE INDEX IF NOT EXISTS idx_incidents_reported_date ON incidents(reported_date DESC);
...
-- Insert sample incidents (for safety risk calculation)
...
DO $$
BEGIN
    RAISE NOTICE '✅ Professional Agent Architecture tables created successfully!';
